...
TEST_CASE_METHOD(sessionFixture, "SessionTestCase")
{
  ...
  SECTION("registerSession")
  {
    ...
    Session* pSession = new Session
      ( [this](){ return now; }, *this, factory, SessionID( BeginString( "FIX.4.2" ),
        SenderCompID( "TW" ), TargetCompID( "ISLD" ) ), provider,
        TimeRange(UtcTimeOnly(), UtcTimeOnly()), 0, 0 );

    CHECK( nullptr == Session::registerSession( 
      SessionID( BeginString( "FIX.4.1" ),
        SenderCompID( "TW" ), TargetCompID( "ISLD" ) ) ) );
    CHECK( pSession == Session::registerSession( 
      SessionID( BeginString( "FIX.4.2" ),
        SenderCompID( "TW" ), TargetCompID( "ISLD" ) ) ) );
    CHECK( Session::isSessionRegistered( 
      SessionID( BeginString( "FIX.4.2" ),
        SenderCompID( "TW" ), TargetCompID( "ISLD" ) ) ) );
    CHECK( nullptr == Session::registerSession( 
      SessionID( BeginString( "FIX.4.2" ),
  ...
  SECTION("next_IOExceptionHandledAndNotLoggedOn") 
  {
    ...
    Session sessionObject = Session( [this](){ return now; }, *this, exceptionFactory, sessionIDCustom, providerCustom,
